                ...
                <div class="card-body">
                  <div class="table-responsive">
					...
					<?php 
						...
						<tr>
						<td>{{ $i }}</td>
						<td >@if ($value->start) {{$value->start}} @else N/A @endif</a></td>
						<td >@if ($value->destination) {{$value->destination}} @else N/A @endif</td>
						<td >@if ($value->earning) {{$value->earning}} @else N/A @endif</td>
						<?php 
						//$driver_id = $value->id;
						
						$doc = \DB::table('driverdocument')->where('driver_id',$id)->get();
						
						?>
						@foreach($doc as $val)
						<td >
						<a href="{{url('public/driverdoc')}}/{{$val->vehicle_insurance}}" target="_blank">{{$val->vehicle_insurance}}</a></td>
						
						
						<td><a href="{{url('public/driverdoc')}}/{{$val->driving_license}}" target="_blank">
						{{$val->driving_license}}</a></td><td><a href="{{url('public/driverdoc')}}/{{$val->driver_id_proof}}" target="_blank">{{$val->driver_id_proof}}</a></td>
